...
def calindicator(ticker):
    ...
    df['rsi'] = (100 - (100 / (1+RS)))
    # df['bear_fractal'] = (
    #         df['high'].shift(4).lt(df['high'].shift(2)) &
    #         df['high'].shift(3).lt(df['high'].shift(2)) &
    #         df['high'].shift(1).lt(df['high'].shift(2)) &
    #         df['high'].lt(df['high'].shift(2))
    # )
    #
    # df['bull_fractal'] = (
    #         df['low'].shift(4).gt(df['low'].shift(2)) &
    #         df['low'].shift(3).gt(df['low'].shift(2)) &
    #         df['low'].shift(1).gt(df['low'].shift(2)) &
    #         df['low'].gt(df['high'].shift(2))
    # )
    periods = (-2, -1, 1, 2)
...
#
        ...
        if indicator['slow_k'][-1]-indicator['slow_d'][-1] > 0 and indicator['slow_k'][-2]-indicator['slow_d'][-2] < 0:
            if not has_item(code2):
                if krw > 5000:
                    upbit.buy_market_order(ticker2, 10000)
                    print("매수!!")
