...
while True:
    try:
        ...
        df = df.sort_values(by='trade_vol', ascending=False).head(40)
        ...
        for i in range(len(df)):
            ...
            if i == 0 or i == 20:
                for j in range(1, len(upbit.get_balances())):
                    code2 = upbit.get_balances()[j]['currency']
                    ticker2 = ('KRW-' + code2)
                    if code2 == 'APENFT':
                        continue
                    indicator = calindicator(ticker2)
                    clo = indicator['close']
                    time.sleep(0.02)
                    wmacd = macd(ticker2)
                    # if code = ''
                        ...
                        if indicator['slow_k'][-1] < indicator['slow_d'][-1] and indicator['slow_k'][-2] > indicator['slow_d'][-2]  and indicator['macdo'][-1] - indicator['macdo'][-2] < 0 :
                            if dif_rate > 0.5:
                                if (sell * current_price) > 5000:
                                    upbit.sell_market_order(ticker2, sell)
                                    print(f'{ticker2} few profit sell!!')
                            elif dif_rate < -5:
                                if (sell * current_price) > 5000:
                                    upbit.sell_market_order(ticker2, sell)
                                    print(f'{ticker2} loss sell!!')
                            else:
                                print(f"{ticker2} selling check....")
            ...
            if not has_item(code):
                ...
                if wmacd['slow_k'][-1] > wmacd['slow_k'][-2] :
                    if indicator['macdo'][-1] - indicator['macdo'][-2] > 0 and indicator['slow_k'][-1] > indicator['slow_d'][-1] and indicator['slow_k'][-2] < indicator['slow_d'][-2] and indicator['slow_k'][-1] < 80:
                        if krw > 300000:
